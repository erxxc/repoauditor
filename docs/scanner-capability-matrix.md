...
| **Weak RNG** (`weak_rng`) | Java `java.util.Random`, `Math.random`, and non-secure Apache Commons `RandomStringUtils` idioms. | At least one non-test `.java` file in the exact snapshot-bound retrieval index. | Offline deterministic candidates with stable file, line, enclosing-symbol, and idiom identities; SecureRandom and secure variants are excluded. | Syntactic Java-only coverage. A match does not prove security-sensitive use, attacker observation, state recovery, or exploitability. |
| **Weak RNG (Python)** (`weak_rng_py`) | Direct textual Python `random.getrandbits`, `random.Random(...)`, and selected `random` module calls. | At least one non-test `.py` file in the exact snapshot-bound retrieval index; target count excludes canonical test paths. | Offline deterministic candidates with stable identities; `secrets` and `random.SystemRandom` are excluded. | Substring-only Python coverage: aliases are missed, and shadowed identifiers or string literals can match. A match does not prove security-sensitive use, attacker observation, state recovery, or exploitability. |
