"""Bounded score-blind renderer for the frozen OPT-003 wave-two packet."""
...
EXPECTED_PLAN_SHA256 = "483eb11d6bd06e7bc9700cb1be342fd2cc1f34c1b0746f20a2c92cd2f80c0d4e"
...
def _stored_rows(db_path: Path, finding_ids: list[int]) -> dict[int, sqlite3.Row]:
    ...
    if len(rows) != len(finding_ids):
        raise RuntimeError("selected wave-two findings do not resolve uniquely in the store")
...
def render_review(plan_path: Path, *, context_lines: int = 20) -> str:
    ...
    if _sha256(plan_path) != EXPECTED_PLAN_SHA256:
        raise RuntimeError("wave-two review plan digest drifted")
    plan = json.loads(plan_path.read_text(encoding="utf-8"))
    entries = plan.get("entries", [])
    if plan.get("packet_size") != 18 or len(entries) != 18:
        raise RuntimeError("wave-two review packet must contain exactly 18 entries")
    config = get_config()
    rows = _stored_rows(config.db_path, [entry["finding_id"] for entry in entries])
    lines = [
        "# OPT-003 wave-two prospective review packet",
        "",
    ...
    for index, entry in enumerate(entries, 1):
        ...
        root = (
            config.paths.data_dir / "raw" / row["repo_id"] / row["commit_hash"]
        ).resolve()
